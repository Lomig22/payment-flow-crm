-- ============================================================
-- Sprint 10 — Stats membre (page Équipe → bouton « Stats »)
--   Enrichit get_user_performance :
--     • stats : ajout relances, relances 2, perdus, qualité (chaud/tiède/
--       froid), taux de no-show — en plus des KPI existants.
--     • by_source : répartition total / clients par canal (cold_call, instagram…)
--     • monthly : ajout des RDV par mois (6 derniers mois)
--   Toutes les stats sont calculées sur la fenêtre p_days (sélecteur de période
--   côté UI), sauf monthly qui reste sur les 6 derniers mois.
  ...
  RETURN json_build_object(
    'stats', (SELECT json_build_object(
      ...
      'no_show_rate',         ROUND(CASE WHEN COUNT(*) FILTER (WHERE appointment_taken = true) > 0
                                ...
                                ELSE 0 END, 1)
    ) FROM leads WHERE setter_id = p_user_id AND created_at >= CURRENT_DATE - p_days),

    -- Répartition par canal d'acquisition sur la fenêtre.
    'by_source', COALESCE((SELECT json_agg(s) FROM (
      SELECT
        source,
        COUNT(*)                                  as total,
        COUNT(*) FILTER (WHERE status = 'client') as clients
      FROM leads
      WHERE setter_id = p_user_id AND created_at >= CURRENT_DATE - p_days
      GROUP BY source
      ORDER BY total DESC
    ) s), '[]'::json),

    -- Évolution sur les 6 derniers mois (indépendant de la fenêtre).
