-- ============================================================
-- Sprint 6 — Scoper le dashboard Cold Call sur source = 'cold_call'
-- get_dashboard_stats agrégeait sur TOUS les leads (cold_call + instagram),
-- ce qui faisait fuiter des données Instagram (ex: RDV pris) dans le
-- dashboard Cold Call. À exécuter dans Supabase > SQL Editor (ou via psql -f)
-- ============================================================
  ...
  RETURN json_build_object(
    ...
    'timeline', COALESCE((
      SELECT json_agg(t) FROM (
        ...
        LEFT JOIN leads l ON l.source = 'cold_call' AND (p_setter_id IS NULL OR l.setter_id = p_setter_id)
...
-- Dashboard Cold Call — "Classement des setters" : idem, scope sur source = 'cold_call'
