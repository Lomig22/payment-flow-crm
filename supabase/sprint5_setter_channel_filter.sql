-- ============================================================
-- Sprint 5 — Filtrer les sections "par setter" des dashboards
-- selon les canaux (acquisition_sources) assignés à chaque setter
-- À exécuter dans Supabase > SQL Editor (ou via psql -f)
-- ============================================================

-- Dashboard Cold Call — "Performance par setter"
...
BEGIN
  ...
  FROM leads
  WHERE created_at >= v_start AND (p_setter_id IS NULL OR setter_id = p_setter_id);
  ...
  RETURN json_build_object(
    ...
    'by_setter', COALESCE((
      SELECT json_agg(s) FROM (
        ...
        FROM users u
        LEFT JOIN leads l ON l.setter_id = u.id AND l.created_at >= v_start
    ...
    'timeline', COALESCE((
      SELECT json_agg(t) FROM (
        SELECT
          ...
          COUNT(l.id) FILTER (WHERE DATE(l.created_at) = d::date AND l.status = 'client') as clients,
          COUNT(l.id) FILTER (WHERE DATE(l.created_at) = d::date AND l.appointment_taken = true) as appointments
        FROM generate_series(v_start, CURRENT_DATE, '1 day') d
        LEFT JOIN leads l ON TRUE AND (p_setter_id IS NULL OR l.setter_id = p_setter_id)
...
-- Dashboard Cold Call — "Classement des setters"
  ...
  RETURN COALESCE((
    SELECT json_agg(s) FROM (
      SELECT
        ...
        ROUND(CASE WHEN COUNT(l.id) > 0 THEN COUNT(l.id) FILTER (WHERE l.status = 'client')::numeric / COUNT(l.id) * 100 ELSE 0 END, 1) as conversion_rate
      FROM users u
      LEFT JOIN leads l ON l.setter_id = u.id AND l.created_at >= CURRENT_DATE - p_days
      WHERE u.role = 'setter' AND u.is_active = true
        AND 'cold_call' = ANY(u.acquisition_sources)
      GROUP BY u.id, u.first_name, u.last_name
...
-- Dashboard Instagram — "Activité par setter"
